    ...
    for sf = 1:NoFitSpecNames
        for sn = 1:size(MRSCont.fit.results.(FitSpecNames{sf}).fitParams,3)
            for sb = 1:size(MRSCont.fit.results.(FitSpecNames{sf}).fitParams,1)
                if ~any(strcmp(FitSpecNames{sf}, {'ref','w'}))
    ...
    for sf = 1:NoFitSpecNames
        for sn = 1:size(MRSCont.fit.results.(FitSpecNames{sf}).fitParams,3)
            for sb = 1:size(MRSCont.fit.results.(FitSpecNames{sf}).fitParams,1)
                if ~any(strcmp(FitSpecNames{sf}, {'ref','w'}))
                    if ~isempty(MRSCont.fit.results.(FitSpecNames{sf}).fitParams{sb,1,sn})
                        FitSpecNamesStruct.(FitSpecNames{sf}){sb,sn} = MRSCont.fit.results.(FitSpecNames{sf}).fitNames{sn};
                    end
                else
                    FitSpecNamesStruct.(FitSpecNames{sf}){1} = MRSCont.fit.results.(FitSpecNames{sf}).fitNames{sn};
                end
            end
        end
    end
                    ...
                    switch MRSCont.opts.fit.method %Which model was used
                        ...
                        case 'Osprey'
                            if ~strcmp(FitSpecNames{ss}, 'ref') && ~strcmp(FitSpecNames{ss}, 'w') && ~strcmp(FitSpecNames{ss}, 'mm') % metabolite only
                                ...
                                dataToPlot = op_takesubspec(MRSCont.processed.(FitSpecNames{ss}){kk}, find(strcmp(MRSCont.processed.(FitSpecNames{ss}){kk}.names, ...
                                    FitSpecNamesStruct.(FitSpecNames{ss}){bf,sf})));
                        ...
                        case 'LCModel'
                            ...
                            % Get the LCModel plots we previously extracted from .coord etc.
                            ModelOutput = fit_LCModelParamsToModel(fitParams);

                            MRSCont.fit.scale{kk} = max(real(dataToPlot.specs)) ./ max(ModelOutput.data);
                            if sf == 1
                                MRSCont.fit.scale_1{kk} = MRSCont.fit.scale{kk};
                            elseif sf == 2
                                MRSCont.fit.scale_2{kk} = MRSCont.fit.scale{kk};
                            elseif sf == 3
                                MRSCont.fit.scale_3{kk} = MRSCont.fit.scale{kk};
                            end

                            % MM (250704)
                            if sf == size(FitSpecNamesStruct.(FitSpecNames{ss}),2)
                                MRSCont.fit.tCr_ampl(kk) = max(ModelOutput.data(ModelOutput.ppm < 3.1 & ModelOutput.ppm > 2.9));
                            end

                    end

                    if ~any(strcmp(FitSpecNames{ss}, {'ref','w','mm'})) % metabolite only
                        ...
                        MRSCont.QM.relAmpl.([FitSpecNames{ss} '_' FitSpecNamesStruct.(FitSpecNames{ss}){1,sf}])(bf,kk) = ...
                            sum(ModelOutput.residual.^2) / (std(real(noise))^2 * length(ModelOutput.residual));
                    end
