...
if ~isfield(MRSCont.opts.fit, 'customLCModelBinary')
    ...
    pathLCModelBinary = fullfile(MRSCont.ospFolder, 'libraries', 'LCModel', pathLCModelBinaryStruct.os, pathLCModelBinaryStruct.osver, ...
        ['LCModel_' pathLCModelBinaryStruct.os, '_', pathLCModelBinaryStruct.osver]);
...
elseif MRSCont.flags.isHERMES || MRSCont.flags.isHERCULES % MM
    callLCModel(MRSCont.opts.fit.lcmodel.controlfileDiff1{kk}, pathLCModelBinaryFile);
    MRSCont.fit.results.metab.fitParams{1,kk,1} = readLCMFitParams(MRSCont, 'diff1', kk);
    callLCModel(MRSCont.opts.fit.lcmodel.controlfileDiff2{kk}, pathLCModelBinaryFile);
    MRSCont.fit.results.metab.fitParams{1,kk,2} = readLCMFitParams(MRSCont, 'diff2', kk);
    callLCModel(MRSCont.opts.fit.lcmodel.controlfileSum{kk}, pathLCModelBinaryFile);
    MRSCont.fit.results.metab.fitParams{1,kk,3} = readLCMFitParams(MRSCont, 'sum', kk);
end
