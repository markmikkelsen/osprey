...
if MRSCont.flags.didFit
    ...
    if ~strcmp(MRSCont.opts.fit.method,'LCModel')
        ...
        elseif MRSCont.flags.isHERMES || MRSCont.flags.isHERCULES
            FitSpecNamesStruct.(FitSpecNames{1}){1} = 'diff1';
            FitSpecNamesStruct.(FitSpecNames{1}){2} = 'diff2';
            FitSpecNamesStruct.(FitSpecNames{1}){3} = 'sum';
        end
    end
...
for rr = 1:Voxels
    for ss = 1:NoSpec %loop over subspectra
        ...
        for g = 1:length(names) % loop over groups
            ...
            MRSCont.overview.Osprey.(['sort_data_voxel_' num2str(rr)]).(names{g}).(['mean_' dataPlotNames{ss}]) = squeeze(mean(real(tempSubSpec),1,'omitnan'));
            MRSCont.overview.Osprey.(['sort_data_voxel_' num2str(rr)]).(names{g}).(['sd_' dataPlotNames{ss}]) = squeeze(std(real(tempSubSpec),0,1,'omitnan'));
        end
        %Store ppm
                ...
                for sf = 1:size(MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(ModelCombs{sc}),3)
                    for bf = 1:size(MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(ModelCombs{sc}),1)
                        if isstruct(MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(ModelCombs{sc}){bf,1,sf})
                            ...
                            %Calculate mean and SD
                            MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['mean_fit_' ModelCombs{sc}])(bf,:,sf) = mean(real(tempSubSpec),1,'omitnan');
                            MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['sd_fit_' ModelCombs{sc}])(bf,:,sf) = std(real(tempSubSpec),0,1,'omitnan');
                            MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['mean_res_' ModelCombs{sc}])(bf,:,sf) = mean(real(tempSubRes),1,'omitnan');
                            MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['sd_res_' ModelCombs{sc}])(bf,:,sf) = std(real(tempSubRes),0,1,'omitnan');
                            MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['mean_data_' ModelCombs{sc}])(bf,:,sf) = mean(real(tempSubdata),1,'omitnan');
                            MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['sd_data_' ModelCombs{sc}])(bf,:,sf) = std(real(tempSubdata),0,1,'omitnan');
                            ...
                            if ~(strcmp(ModelCombs{sc}, 'ref') || strcmp(ModelCombs{sc}, 'w')) %Is not water
                                MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['mean_baseline_' ModelCombs{sc}])(bf,:,sf) = mean(real(tempSubBaseline),1,'omitnan');
                                MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['sd_baseline_' ModelCombs{sc}])(bf,:,sf) = std(real(tempSubBaseline),0,1,'omitnan');
                                for f = 6:length(fits) % loop over basis functions
                                    MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['mean_' fits{f} '_' ModelCombs{sc}])(bf,:,sf) = mean(real(tempInidivMetab.(fits{f})),1,'omitnan');
                                    MRSCont.overview.Osprey.(['sort_models_voxel_' num2str(rr)]).(names{g}).(['sd_' fits{f} '_' ModelCombs{sc}])(bf,:,sf) = std(real(tempInidivMetab.(fits{f})),0,1,'omitnan');
                                end
                            end
