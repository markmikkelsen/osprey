...
switch Module
    ...
    case 'OspreyFit'
        ...
        switch MRSCont.opts.fit.method
            case 'LCModel'
                ...
                elseif MRSCont.flags.isMEGA
                    subspectrum_names = {'A','diff1'};
                elseif MRSCont.flags.isHERMES || MRSCont.flags.isHERCULES
                    subspectrum_names = {'diff1','diff2','sum'};
                end
                % Number of metabolites and lipid/MM basis functions
                basisNames = MRSCont.fit.results.metab.fitParams{kk}.name;
                idx = find(strcmp(basisNames, {'MM09'})); % MM (250410)
                if ~isscalar(idx)
                    basisNames(idx(1)) = [];
                    MRSCont.fit.results.metab.fitParams{kk}.name(idx(1)) = [];
                    MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.CRLB(idx(1))        = [];
                    MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.relConc(idx(1))     = [];
                    MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.indivMets(:,idx(1)) = [];
                    MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.ampl(idx(1))        = [];
                end
                % nLip    = sum(~cellfun(@isempty, strfind(basisNames, 'Lip')));
                % nMM     = sum(~cellfun(@isempty, strfind(basisNames, 'MM')));
                nLip    = sum(contains(basisNames, 'Lip'));
                nMM     = sum(contains(basisNames, 'MM'));
                nMMLip  = nLip + nMM;
                nMets   = length(basisNames) - nMMLip;
                % nComb   = sum(~cellfun(@isempty, strfind(basisNames, '_')));
                nComb   = sum(contains(basisNames, '_'));
                ...
                idx = find(strcmp(basisSetNames, {'MM09'})); % MM (250410)
                if ~isscalar(idx)
                    basisSetNames(idx(1)) = [];
                end
                subSpecName = subspectrum_names{subspectrum};
        ...
        if ~strcmp(MRSCont.opts.fit.style, 'Concatenated') ||  strcmp(which, 'ref') || strcmp(which, 'w') %Is not concatenated or is reference/water fit
            switch MRSCont.opts.fit.method
                case 'LCModel'
                    if strcmp(which, 'ref') || strcmp(which, 'w')
        ...
        if ~(isfield(MRSCont.flags,'isPRIAM') || isfield(MRSCont.flags,'isMRSI')) || ~(MRSCont.flags.isPRIAM || MRSCont.flags.isMRSI)
            ...
            % idx = find(strcmp(MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.name, {'MM09'})); % MM (250410)
            % if ~isscalar(idx)
            %     MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.name(idx(1))        = [];
            %     MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.CRLB(idx(1))        = [];
            %     MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.relConc(idx(1))     = [];
            %     MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.indivMets(:,idx(1)) = [];
            %     MRSCont.fit.results.(which).fitParams{basis,kk,subspectrum}.ampl(idx(1))        = [];
